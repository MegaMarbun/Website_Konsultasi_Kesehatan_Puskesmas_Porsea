        ...
        <div class="row g-5">
            <div class="col-lg-6 wow fadeIn" data-wow-delay="0.1s">
                <div class="d-flex flex-column">
                    ...
                    <p style="color: black;">Pemeriksaan kesehatan umum umumnya melibatkan riwayat kesehatan, pemeriksaan fisik (singkat atau lengkap) dan terkadang tes laboratorium.</p>
                </div>
            </div>
        </div>
            ...
            <form action="{{ url('konsultasi_umum/edit/'.$konsul->id) }}" method="POST">
                ...
                <form action="{{ url('konsultasi_umum') }}" method="POST">
                    ...
                    <div class="mb-3 mt-3">
                        ...
                        <input type="text" class="form-control @error('username') is-invalid @enderror" name="username" id="username" placeholder="Masukkan username Anda" value="{{ old('username',Auth::user()->username) }}">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3 mt-3">
                        ...
                        <input type="number" min="1" class="form-control" placeholder="Masukkan usia" name="usia" value="{{ old('usia', $konsul->usia) }}">
                    </div>
                    <div class="mb-3 mt-3">
                        <label for="email" class="form-label">Berat Badan (Kg)* :</label>
                        <input type="number" min="1" class="form-control" placeholder="Masukkan berat badan anda" name="berat_badan" value="{{ old('berat_badan', $konsul->berat_badan) }}">
                    </div>
                    <div class="mb-3 mt-3">
                        <label for="email" class="form-label">Tinggi Badan (cm) :</label>
                        <input type="number" min="1" class="form-control" placeholder="Masukkan Tinggi anda" name="tinggi_badan" value="{{ old('tinggi_badan', $konsul->tinggi_badan) }}">
                    </div>
                    <div class="mb-3 mt-3">
                        <label for="email" class="form-label">Golongan Darah:</label>
                        <input type="text" class="form-control" placeholder="Masukkan golongan darah anda" name="gol_darah" value="{{ old('gol_darah', $konsul->gol_darah) }}">
                    </div>
                    <div class=" mb-3">
                        <label for="comment">Keluhan*:</label>
                        <textarea class="form-control" rows="5" id="comment" name="keluhan" placeholder="ketikkan keluhan anda">{{ old('keluhan', $konsul->keluhan) }}</textarea>
                    </div>
                    <center>
